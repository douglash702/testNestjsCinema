...
export default function () {
    ...
    const movies = JSON.parse(getMovies.body);
    if (movies.length === 0) {
    ...
    const getMoviesId = baseRestInstance.get(`${ENDEPOINTS.MOVIE_ENDPOINT}/${movie._id}`);
    baseChecks.checkResponseTime(getMoviesId, 50, 'GET ID /movies - Tempo de resposta' )
